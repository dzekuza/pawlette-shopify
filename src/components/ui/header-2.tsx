  ...
  return (
    <header
      className={cn(
        ...
        {
          'bg-background/95 supports-[backdrop-filter]:bg-background/50 border-border border-b backdrop-blur-lg md:top-4 md:shadow':
            scrolled && !open,
    ...
    >
      ...
      >
        <WordmarkIcon className="h-4 shrink-0" />
        <div className="hidden items-center gap-2 md:absolute md:left-1/2 md:flex md:-translate-x-1/2">
          {links.map((link, i) => (
            <a
              key={i}
        ...
        >
          <div className="grid gap-y-2">
            {links.map((link) => (
              <a
                key={link.label}
                className="inline-flex h-10 items-center justify-start rounded-md px-4 text-sm font-medium text-foreground transition-colors hover:bg-accent hover:text-accent-foreground"
                href={link.href}
              >
                {link.label}
              </a>
